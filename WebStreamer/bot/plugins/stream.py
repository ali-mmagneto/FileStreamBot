...
async def private_receive_handler(c: Client, m: Message):
    ...
    try:
        ...
        msg_text ="""
<i><u>Link Dönüştürüldü!</u></i>\n
<b>📂 Dosya Adı :</b> <i>{}</i>\n
<b>📦 Dosya Boyutu :</b> <i>{}</i>\n
<b>📥 İndirme :</b> <i>{}</i>\n
<b>🚸 Not : Tʜɪs ᴘᴇʀᴍᴀɴᴇɴᴛ Lɪɴᴋ, Nᴏᴛ Exᴘɪʀᴇᴅ</b>\n
<i>© @mmagneto3 </i>"""
...
async def channel_receive_handler(bot, broadcast):
    ...
    try:
        ...
        await bot.edit_message_reply_markup(
            ...
            reply_markup=InlineKeyboardMarkup(
                [[InlineKeyboardButton("İndirme Linki 📥", url=f"https://t.me/{(await bot.get_me()).username}?start=AvishkarPatil_{str(log_msg.id)}")]])
            # [[InlineKeyboardButton("İndirme Linki 📥", url=f"https://t.me/banavidyoatbot?start=AvishkarPatil_{str(log_msg.id)}")]])
        )
